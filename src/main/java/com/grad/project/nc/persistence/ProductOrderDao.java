...
public class ProductOrderDao extends AbstractDao<ProductOrder> {
    ...
    public ProductOrder add(ProductOrder entity) {
        KeyHolder keyHolder = executeInsert(connection -> {
            String statement = "INSERT INTO product_order (product_instance_id, user_id, order_aim_id, " +
            ...
            preparedStatement.setLong(3, entity.getOrderAimId());
    ...
    public ProductOrder update(ProductOrder entity) {
        executeUpdate(connection -> {
            String statement = "UPDATE product_order SET product_instance_id=?, user_id=?, order_aim_id=?, " +
    ...
    public ProductOrder find(long id) {
        return findOne(connection -> {
            String statement = "SELECT product_order_id, product_instance_id, user_id, order_aim_id, " +
    ...
    Collection<ProductOrder> getOrdersByUser(User userProxy) {return findMultiple(connection -> {
        String statement = "SELECT product_order_id, product_instance_id, product_order.user_id, order_aim_id, " +
                "status_id, responsible_id, open_date, close_date FROM product_order " +
