...
orders with: order number; product ordered; price; date of order.',
   ...
   'WITH completed_user_orders AS (
    ...
    completed_create_user_orders AS (
      SELECT
      ...
      WHERE cuo."order_aim_Id" = 13
  ),
    completed_suspend_user_orders AS (
      SELECT cuo.order_number                                             AS "Number",
             cuo.order_aim                                                AS "Aim",
             cuo.product                                                  AS "Product",
             cuo.open_date                                                AS "Open",
             cuo.close_date                                               AS "Close",
             ...
             cuo.discount || ''%''                                          AS "Discount"
      FROM completed_user_orders cuo
      WHERE cuo."order_aim_Id" = 14
  ),
    completed_deactivate_user_orders AS (
      SELECT cuo.order_number                                             AS "Number",
             cuo.order_aim                                                AS "Aim",
             cuo.product                                                  AS "Product",
             cuo.open_date                                                AS "Open",
             ...
             cuo.discount || ''%''                                          AS "Discount"
      FROM completed_user_orders cuo
      WHERE cuo."order_aim_Id" = 15
  ),
    completed_resume_user_orders AS (
      SELECT cuo.order_number                                             AS "Number",
             cuo.order_aim                                                AS "Aim",
             cuo.product                                                  AS "Product",
             cuo.open_date                                                AS "Open",
             ...
             cuo.discount || ''%''                                          AS "Discount"
      FROM completed_user_orders cuo
      WHERE cuo."order_aim_Id" = 25
  )
SELECT *
FROM completed_create_user_orders
UNION ALL
SELECT *
FROM completed_suspend_user_orders
UNION ALL
SELECT *
FROM completed_deactivate_user_orders
UNION ALL
SELECT *
FROM completed_resume_user_orders;',
   '[
     {
       "id": 0,
       "name": "User e-mail",
